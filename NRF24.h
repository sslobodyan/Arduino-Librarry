...
#define ACTIVATE      		0x50 // Для клона 
...
class NRF24
{
	public:
		...
		uint8_t* local_address=0; // Собственный адрес
		uint8_t* remote_address=0; // Адрес удалённой стороны
		uint8_t channel = 101;	// частотный канал
		uint8_t* rx_buffer;	// куда принимать пакет
		uint8_t* tx_buffer;	// какой хотим отправить новый пакет
		uint8_t tx_buffer_len;	// длина отправляемых данных
		bool 	go_tx;	// флаг что нужно отправить новый пакет
		
		void (*RX_handler)(uint8_t len) = 0; // указатель на функцию-обработчик принятого пакета
		void (*TX_handler)(void) = 0; // указатель на функцию-обработчик после отсылки пакета

		NRF24(uint8_t _cepin, uint8_t _cspin);
		NRF24(uint8_t _cepin, uint8_t _cspin, uint8_t _irqpin);
		
		void check(void);
		void csn_assert(void);
		void csn_deassert(void);
		void assert_ce(void);
		void deassert_ce(void);
		uint8_t update_local_address();
		uint8_t update_remote_address();
		uint8_t update_channel();
		uint8_t read_buf(uint8_t cmd, uint8_t * buf, uint8_t count);
		uint8_t write_buf(uint8_t cmd, uint8_t * buf, uint8_t count);
		uint8_t readreg(uint8_t reg);
		uint8_t writereg(uint8_t reg, uint8_t val);
		uint8_t readreg_buf(uint8_t reg, uint8_t * buf, uint8_t count);
		uint8_t writereg_buf(uint8_t reg, uint8_t * buf, uint8_t count);
		uint8_t read_rx_payload_width(void);
		...
		uint8_t flushRX();
		uint8_t flushTX();
		uint8_t get_data(uint8_t * buf);
		uint8_t send_data(uint8_t * buf, uint8_t size);
		void print_registers(void);
		void listening();
		void activate(uint8_t code);
		bool testCarrier(void);
		void getAddr(uint8_t *buf, bool local);
		uint8_t get_status();
		void pwrdown();
		void handler();
		
	protected:
		uint8_t cepin, cspin, irqpin;
			
	private:
		bool use_irq_pin = false;
		void delay_15us();
		uint8_t spi_send_recv(uint8_t data);
		uint8_t is_interrupt_with_irq_pin();
		uint8_t is_interrupt_no_pin();
};
